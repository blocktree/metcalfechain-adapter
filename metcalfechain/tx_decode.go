...
import (
	...
	"errors"
	"fmt"
	"github.com/blocktree/metcalfechain-adapter/metcalfechainTransaction"
	"math/big"
	"sort"
	"time"
...
func (decoder *TransactionDecoder) CreateMRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if !isActived && amount.Cmp(big.NewInt(decoder.wm.Config.ReserveAmount)) < 0 {
		return errors.New("The destination address [" + to + "] is not actived yet, if you owned the address, you need to send at least 20 M to active it, these 20 M will be locked forever!")
	}
	...
	for _, a := range addressesBalanceList {
		...
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				decoder.wm.Log.Std.Notice("The M of the account is enough," +
	...
	emptyTrans, hash, err := metcalfechainTransaction.CreateEmptyRawTransactionAndHash(from, fromPub, destinationTag, sequence, to, convertFromAmount(amountStr), fee, uint32(blockHeight)+uint32(decoder.wm.Config.LastLedgerSequenceNumber), decoder.wm.Config.MemoType, tagStr, decoder.wm.Config.MemoFormat)
	if err != nil {
		return err
	}
...
func (decoder *TransactionDecoder) SignMRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			///////交易单哈希签名
			signature, err := metcalfechainTransaction.SignRawTransaction(keySignature.Message, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyMRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	pass, signedTrans := metcalfechainTransaction.VerifyAndCombinRawTransaction(emptyTrans, signature, pubkey)
...
func (decoder *TransactionDecoder) CreateSimpleSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransaction, error) {
	...
	for _, addrBalance := range addrBalanceArray {
		...
		if !isActived && sumAmount_BI.Cmp(big.NewInt(decoder.wm.Config.ReserveAmount)) < 0 {
			return nil, errors.New("The summary address [" + sumRawTx.SummaryAddress + "] is not actived yet, if you owned the address, you need to send at least 20 M to the address to active it, these 20 M will be locked forever!")
		}
...
func (decoder *TransactionDecoder) createRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction, addrBalance *openwallet.Balance) error {
	...
	emptyTrans, hash, err := metcalfechainTransaction.CreateEmptyRawTransactionAndHash(from, fromAddr.PublicKey, int64(destinationTag), sequence, to, convertFromAmount(amountStr), fee, uint32(currentHeight)+uint32(decoder.wm.Config.LastLedgerSequenceNumber), decoder.wm.Config.MemoType, tagStr, decoder.wm.Config.MemoFormat)
