...
func (c *Client) NewTransaction(json *gjson.Result, memoScan string) *Transaction {
	...
	amount, _ := strconv.ParseInt(gjson.Get(json.Raw, "meta").Get("delivered_amount").String(), 10, 64)
	//strconv.ParseInt(gjson.Get(json.Raw, "Amount").String(), 10, 64)
	//if err == nil {
	...
	if len(memos) > 0 {
		memo, _ := hex.DecodeString(memos[0].Get("Memo").Get("MemoData").String())
		obj.Memo = string(memo)
	}
	return obj
}
...
func (c *WSClient) NewTransaction(json *gjson.Result, memoScan string) *Transaction {
	...
	//	fmt.Println("[M:wrong_status] tx-",json.String())
	//}
